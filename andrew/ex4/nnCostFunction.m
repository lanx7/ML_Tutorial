...
y_onehot = full(sparse(y, 1:m,1,num_labels,m));
size(y_onehot);

reg_term1 = sum(sum(Theta1(:,2:end) .^ 2)) ;
reg_term2 = sum(sum(Theta2(:,2:end) .^ 2));
reg_term = (reg_term1 + reg_term2 ) * lambda / (2 * m);
J = sum(sum((-y_onehot .* log(hypothesis')) - ((1-y_onehot) .* log(1-hypothesis')))) / m + reg_term;
...
Theta1_grad = delta1 / m + grad_reg1;
Theta2_grad = delta2 / m + grad_reg2;
