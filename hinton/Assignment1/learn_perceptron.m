...
fprintf(['mistakes0',mat2str(mistakes0), '\n']);
fprintf(['mistakes1',mat2str(mistakes1), '\n']);
...
%    w = update_weights(neg_examples,pos_examples,w);
    ...
    w = update_weights_gradient(neg_examples,pos_examples,w);
...
function [w] = update_weights_gradient(neg_examples, pos_examples, w_current)
alpha = -1
w = w_current;
...
mistakes0 = [];
mistakes1 = [];
% Error in Negative Examples(Circle) 
for i=1:num_neg_examples
    x = neg_examples(i,:)';
    activation = x'*w;
    if (activation >= 0)
...
% Error in Positive Examples(Rect) 
for i=1:num_pos_examples
    x = pos_examples(i,:)';
    activation = x'*w;
    if (activation < 0)
